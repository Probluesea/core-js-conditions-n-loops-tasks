...
function convertToRomanNumerals(num) {
  ...
  switch (str.length) {
    ...
    case 2:
      result += `${repeat(roman[10], str[0])}${roman[num % 10]}`;
      break;
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    for (let j = 0; j < i; j += 1) left += arr[j];
    for (let k = i + 1; k < arr.length; k += 1) right += arr[k];
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < mat.length; i += 1) {
    for (let j = 0; j < Math.floor(mat.length / 2); j += 1) {
      const oldVal = mat[i][j];
      mat[i][j] = mat[i][mat.length - 1 - j];
      mat[i][mat.length - 1 - j] = oldVal;
    }
  }
